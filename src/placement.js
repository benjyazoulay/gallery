...
let authToken = null; // Stocker le token Firebase
...
module.exports = (regl, { placements, getAreaIndex }) => {
    ...
    const loadPainting = (p) => {
        if (!authToken) {
            console.error("Token d'authentification manquant. Impossible de charger les peintures.");
            return; // Sortir si le token est absent
        }

        console.log("Chargement des peintures avec le token:", authToken);
        ...
        console.log("Position de la peinture:", pos);
        ...
        console.log("Peinture ajoutée au batch. Taille du batch:", batch.length);
    };

    auth.onAuthStateChanged(async (user) => {
        if (user) {
            console.log("Utilisateur connecté dans placement.js:", user.email);

            // Récupérer le token Firebase
            authToken = await user.getIdToken();
            console.log("Token Firebase récupéré:", authToken);

            texture.fetch(regl, loadPainting, () => {
                fetching = false;
                console.log("Tous les tableaux ont été récupérés dans placement.js");
            });
        } else {
            console.log("Aucun utilisateur connecté dans placement.js.");
            authToken = null; // Réinitialiser le token si l'utilisateur se déconnecte
            batch = []; // Réinitialiser les peintures
        }
    });

    console.log("Appel de texture.fetch");
    texture.fetch(regl, loadPainting, () => {
        fetching = false;
        console.log("Tous les tableaux ont été récupérés dans placement.js");
    });
